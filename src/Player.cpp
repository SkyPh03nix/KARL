    ...
    : speed(speed), inventory(windowSize, invCap)
{
    ...
    sprite.setOrigin(bounds.width / 2.f, bounds.height / 2.f); //origin to center
...
void Player::initAnimationSet(sf::Texture& texture, const std::string& prefix, int frameCount, float frameTime, int directionsCount) {
    ...
    for (int dir = 0; dir < directionsCount; ++dir) {
        ...
        // auto naming convention: walk_down, walk_up, walk_left, walk_right
        switch (dir) {
...
void Player::update(float deltaTime, const sf::RenderWindow& window) {
    ...
    //Checking for window bounds and correct position if outside (currently disabled because of camera movement)
